...
1. **Append-only; write-only during loops.** This file is history-class
   (see `../../LOOP_ENGINEERING.md`, "Reading discipline"): append your
   entry without reading the ones before it, and never open this file for
...
### 2026-02-24 — Audit evidence granularity underspecified

- Where: phase loop step 3, preparing to close Autofix Phase 1
- What happened: the audit template asks for evidence per gate but not
  whether command output should be quoted verbatim or summarized; guessed
  verbatim on the first draft and the audit ballooned before being trimmed.
- Framework file/rule: `docs/audits/TEMPLATE.md`, "Evidence, Gate by Gate"
- Suggested change: one sentence in the template on evidence granularity.

> Harvested through "Audit evidence granularity underspecified" — filed
> upstream as loop-engine issue #7, 2026-03-02.
