...
The human checkpoint for linkcheck. Drop instructions, corrections, or new
requirements under **Items** below — any time, any format. The agent picks
them up at its next loop boundary, so you can steer a running loop without
interrupting it or re-explaining context in chat.

This file is intentionally empty at rest — what you see here is its normal
state.
